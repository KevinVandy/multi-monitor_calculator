<script lang="ts">
  ...
  export let monitor: IMonitor | null = null;
...
<Dialog bind:open={confirmDeleteMonitorDialogOpen}>
  ...
  <DialogContent>
    <p>
      Are you sure you want to delete monitor <i
        >{monitor?.name || monitor.index + 1}</i
      > ?
    </p>
  </DialogContent>
  <DialogActions>
    <Button on:click={() => (confirmDeleteMonitorDialogOpen = false)}>
      No, Cancel
    </Button>
    <Button
      on:click={() => {
