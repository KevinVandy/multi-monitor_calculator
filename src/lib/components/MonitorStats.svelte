<script lang="ts">
	...
	import { calcPPI, calcScreenHeight, calcScreenWidth, calcTheta } from '$lib/utils/calc';
...
<DataTable style="width: 100%;">
	<Body>
		...
		<Row>
			<Cell
				>{$statUnits === 'Metric' ? 'Pixels Per Centimeter (PPCM)' : 'Pixels Per Inch (PPI)'}</Cell
			>
		...
		<Row>
			...
			<Cell numeric>{screenWidth.toFixed(1)}{$statUnits === 'Metric' ? ' cm' : '"'}</Cell>
		</Row>
		<Row>
			<Cell>Screen Height</Cell>
			<Cell numeric>{screenHeight.toFixed(1)}{$statUnits === 'Metric' ? ' cm' : '"'}</Cell>
		</Row>
		<Row>
			<Cell>Bezel Width</Cell>
			<Cell numeric>{bezelWidth.toFixed(2)}{$statUnits === 'Metric' ? ' cm' : '"'}</Cell>
		</Row>
		<Row>
			<Cell>Physical Width</Cell>
			<Cell numeric>{physicalWidth.toFixed(1)}{$statUnits === 'Metric' ? ' cm' : '"'}</Cell>
		</Row>
		<Row>
			<Cell>Physical Height</Cell>
			<Cell numeric>{physicalHeight.toFixed(1)}{$statUnits === 'Metric' ? ' cm' : '"'}</Cell>
		</Row>
		<Row>
