<script lang="ts">
	...
	import { performSearch, searchOptions } from '$lib/utils/searchEngine';
...
<div class="container">
	<Group variant="raised">
		<Button on:click={() => performSearch($preferredSearchEngine, monitor)} variant="raised">
			...
			<Menu bind:this={menu} anchorCorner="TOP_LEFT">
				<List>
					{#each Object.values(searchOptions.searchEngine) as searchEngine}
						<Item on:SMUI:action={() => performSearch(searchEngine.name, monitor)}>
							<Image
